...
# Test model
test_cdr3b, test_peptide, y_test = list(test_data['cdr3']), list(test_data['peptide']), list(test_data['Binding'])
key_pep = []
dict_tmp = {}
...
if not os.path.exists("t-SNE_NetTCR2"):
    os.makedirs("t-SNE_NetTCR2")
...
for root, dirs, files in os.walk(val_model_path):
    for file in files:
        if file.startswith("min"):
            ...
            output_con = concatenate([cat_cdr3_output_test, cat_peptide_output_test])
            ...
            for k in range(len(key_pep)):
                plt.figure()
                plt.title('without Heterogeneous GNN module: ' + key_pep[k])
                ...
                plt.xlabel("t-SNE 1")
                plt.ylabel("t-SNE 2")
                plt.savefig(os.path.join('t-SNE_NetTCR2', key_pep[k] + '.pdf'), format="pdf")
                plt.close('all')
