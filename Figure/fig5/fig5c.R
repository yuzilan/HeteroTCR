...
p = ggplot(data=df,aes(x=Peptide,y=Value,fill=Model))+
...
#     stat_summary(geom="errorbar",fun.min=min,fun.max=max,width=0.3,position=position_dodge(0.7))+
    labs(x="",y="Average distance to the center",fill="Models")+
...
ggsave("Figure5c_20240502.pdf",p,device = "pdf",width=18,height=7.4,units="cm")
